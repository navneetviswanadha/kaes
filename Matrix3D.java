...
class Matrix3D {
	// local registers for x,y and z, e.g. 3x3x3 matrix
    float xx, xy, xz, xo;
    float yx, yy, yz, yo;
    float zx, zy, zz, zo;
	
    static final double pi = 3.14159265; // ought to take up to double percision!
    ...
    /** Translate the origin -- Should this be scaled? */
   ...
   float[] getOrigin() {
	...
	points as integers; three successive entries per point
		Looks like we could skip the copy phase at the beginning to increase speed
	*/
    void transform(float v[], int tv[], int nvert) {
		...
		//	float lzx = zx, lzy = zy, lzz = zz, lzo = zo;
			for (int i = nvert * 3; (i -= 3) >= 0;) {
			    ...
			    float y = v[i + 1];
			    float z = v[i + 2];
			    tv[i    ] = (int) ((x * xx + y * xy + z * xz + xo)*adjXscale+xMargin);
			    tv[i + 1] = (int) ((x * yx + y * yy + z * yz + yo)*adjYscale+yMargin);
			    tv[i + 2] = (int) ((x * zx + y * zy + z * zz + zo)*adjZscale);
			}
    }
	int xMargin=10, yMargin=10;
	float adjZscale=20, adjXscale=20, adjYscale=20;
	
	public void setAdjscale(float x,float y, float z) {
		adjXscale=x;
		adjYscale=y;
		adjZscale=z;
	}
