...
public class EquationDialog extends Dialog
{
	...
	public EquationDialog(Kaes parent, boolean modal)
	{
		super(parent, modal);
		this.parent = parent;
		...
		messageArea.addKeyListener(aSymKey);
		//}}
	}
	...
	public EquationDialog(Kaes parent, String title, boolean modal)
	{
		this(parent, modal);
		this.parent = parent;
		setTitle(title);
	...
	class SymWindow extends java.awt.event.WindowAdapter
	{
		public void windowClosing(java.awt.event.WindowEvent event)
		{
			...
			if (object == EquationDialog.this)
				EquationDialog_WindowClosing(event);
		}
	}
	
	void EquationDialog_WindowClosing(java.awt.event.WindowEvent event)
	{
		setVisible(false);
	}
	...
	class SymKey extends java.awt.event.KeyAdapter
	{
		...
		{
			Object object = event.getSource();
			if (object == messageArea)
				messageArea_KeyTyped(event);
		}
	}

	void messageArea_KeyTyped(java.awt.event.KeyEvent event)
	{
		// to do: code goes here.
			 
		//{{CONNECTION
		// Append string in TextArea Get the selected text
		String lhsAllowed = "CPI";
		String rhsAllowed = "CPI";
		int c = event.getKeyCode();
		int t;
		String x;
		int ss = messageArea.getSelectionStart();
		int se = messageArea.getSelectionEnd();
		//if (se == ss) se = ss+1;
		int zeroInserted = messageArea.getText().indexOf("0");
		
		t = messageArea.getText().indexOf("=");
		
		x = event.getKeyText(event.getKeyCode());
		if (messageArea.getSelectedText().indexOf("=") != -1) return;
	
		if ((x.equalsIgnoreCase("Delete") || x.equalsIgnoreCase("Backspace")) && (se != ss || ss > 0)) {
			if (ss == se && ss > 0) {
				se = ss;
				ss--;
				messageArea.setSelectionStart(ss);
				messageArea.setSelectionEnd(se);
			}
			if (messageArea.getSelectedText().indexOf("=") != -1) return;
			 messageArea.replaceRange("",ss,se);
		} else  if (t == -1) {// no equals yet do lhs
			if (x.equals("=")) {
				if (ss > 0) messageArea.replaceRange(x,ss,se);
			} else if (lhsAllowed.indexOf(x) != -1)
				messageArea.replaceRange(x,ss,se);
			else {
			...
			if ( zeroInserted < 0 ) {
				if (x.equals("0") && ss == messageArea.getText().length())
						messageArea.insertText(x,ss);
				...
				if (rhsAllowed.indexOf(x)!= -1 &&
					messageArea.getSelectionStart() < zeroInserted)
						messageArea.replaceRange(x,ss,se);
				else {
